...
export function PdfToExcelClient() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    trackEvent({ name: "tool_used", tool: "pdf-to-excel" });
    setDownloadUrl(null);

    try {
      const fileToUpload = new File([file.buffer], file.name, { type: 'application/pdf' });
      
      const { jobId } = await uploadFile('/convert', fileToUpload, { targetFormat: 'xlsx' });
      ...
      showToast("Failed to convert PDF to Excel. Please try again or check your backend connection.", "error");
    } finally {
  ...
  const handleAdComplete = useCallback(() => {
    trackEvent({ name: "download_completed", tool: "pdf-to-excel" });
    setShowAd(false);
  ...
  return (
    <ToolPageLayout
      title="PDF to Excel"
      description="Convert PDF to EXCEL spreadsheets online. Extract data tables securely from your PDF."
      icon={Table}
    ...
    >
      ...
      {!file ? (
        <PdfDropzone
          ...
          sublabel="Select a PDF to extract tables to Excel"
        />
      ) : (
            ...
            {!downloadUrl ? (
              ...
              >
                {isProcessing ? (
                  ...
                  <><RefreshCw className="w-5 h-5" /> Convert to Excel</>
                )}
              </button>
            ) : (
              <button
