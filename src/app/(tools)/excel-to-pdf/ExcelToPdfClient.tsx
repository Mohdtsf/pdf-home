...
import { Table, Download, Loader2, RefreshCw } from "lucide-react";
...
export function ExcelToPdfClient() {
  ...
  const validateExcelFile = useCallback(async (f: File) => {
  ...
  const handleConvert = useCallback(async () => {
    ...
    trackEvent({ name: "tool_used", tool: "excel-to-pdf" });
    setDownloadUrl(null);

    try {
      const fileToUpload = new File([file.buffer], file.name, {
        type: file.file.type || 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      });
      ...
      showToast("Failed to convert Excel to PDF. Please try again or check your backend connection.", "error");
    } finally {
  ...
  const handleAdComplete = useCallback(() => {
    trackEvent({ name: "download_completed", tool: "excel-to-pdf" });
    setShowAd(false);
  ...
  return (
    <ToolPageLayout
      title="Excel to PDF"
      description="Convert Excel spreadsheets to PDF online. Perfect formatting conversion."
      icon={Table}
