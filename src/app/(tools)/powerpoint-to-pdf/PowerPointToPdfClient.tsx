...
export function PowerPointToPdfClient() {
  ...
  const validatePptFile = useCallback(async (f: File) => {
  ...
  const handleConvert = useCallback(async () => {
    ...
    trackEvent({ name: "tool_used", tool: "powerpoint-to-pdf" });
    setDownloadUrl(null);

    try {
      const fileToUpload = new File([file.buffer], file.name, {
        type: file.file.type || 'application/vnd.openxmlformats-officedocument.presentationml.presentation',
      });
      ...
      showToast("Failed to convert PowerPoint to PDF. Please try again or check your backend connection.", "error");
    } finally {
  ...
  const handleAdComplete = useCallback(() => {
    trackEvent({ name: "download_completed", tool: "powerpoint-to-pdf" });
    setShowAd(false);
  ...
  return (
    <ToolPageLayout
      ...
      description="Convert PowerPoint slides to PDF online. Perfect formatting conversion."
      icon={Presentation}
    ...
    >
      ...
      {!file ? (
        <PdfDropzone
          ...
          acceptLabel="PowerPoint files (.ppt, .pptx) only"
          validateFile={validatePptFile}
          label="Drop your PowerPoint file here"
          sublabel="Select a PowerPoint document to convert to PDF"
        />
      ) : (
